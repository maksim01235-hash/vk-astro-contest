...
export default function AdminPage() {
  ...
  /** Замена блока по его текущему id — работает и при переименовании идентификатора. */
  const handleBlockChange = useCallback((blockId: string, nextBlock: Block) => {
    setBlocks((prev) => prev.map((block) => (block.id === blockId ? nextBlock : block)));
    setSelectedId((current) => (current === blockId ? nextBlock.id : current));
  }, []);

  /**
   * Проблемы идентификаторов перед сохранением: пустые значения и дубликаты
   * в своих областях (id — все блоки; answerKey — InputField; zoneId — DragZone;
   * objectId — DragObject). Сохранение блокируется до их устранения.
   */
  const collectIdProblems = (): string[] => {
    const problems: string[] = [];
    const labelOf = (block: Block) => `${block.type} (порядок ${block.order})`;
    const counts: Record<'id' | 'answerKey' | 'zoneId' | 'objectId', Map<string, number>> = {
      id: new Map(), answerKey: new Map(), zoneId: new Map(), objectId: new Map(),
    };
    const count = (scope: keyof typeof counts, value: string | undefined) => {
      if (!value || !value.trim()) return;
      counts[scope].set(value, (counts[scope].get(value) || 0) + 1);
    };

    blocks.forEach((block) => {
      if (!block.id.trim()) problems.push(`${labelOf(block)}: пустой ID блока`);
      count('id', block.id);
      if (block.type === 'InputField') {
        if (!block.answerKey.trim()) problems.push(`${labelOf(block)}: пустой ключ ответа`);
        count('answerKey', block.answerKey);
      }
      if (block.type === 'DragZone') {
        if (!block.zoneId.trim()) problems.push(`${labelOf(block)}: пустой ID зоны`);
        count('zoneId', block.zoneId);
      }
      if (block.type === 'DragObject') {
        if (!block.objectId.trim()) problems.push(`${labelOf(block)}: пустой ID объекта`);
        count('objectId', block.objectId);
      }
    });

    const scopeNames = { id: 'ID блока', answerKey: 'ключ ответа', zoneId: 'ID зоны', objectId: 'ID объекта' } as const;
    (Object.keys(counts) as Array<keyof typeof counts>).forEach((scope) => {
      counts[scope].forEach((n, value) => {
        if (n > 1) problems.push(`Дубликат «${scopeNames[scope]}» "${value}" (${n} шт.)`);
      });
    });
    return problems;
  };
