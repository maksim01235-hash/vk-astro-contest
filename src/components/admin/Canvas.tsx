...
export function Canvas({ blocks, onChange, selectedId, onSelect }: Props) {
  ...
  return (
    <div className="card-surface min-h-[400px]">
      ...
      {blocks.length === 0 ? (
        <div className="py-12 text-center text-slate-400">Добавьте блоки из панели слева</div>
      ) : (
...
function SortableBlock({
  ...
  const summary =
    block.type === 'ImageBlock'
      ...
      : block.type === 'ImageMarkerBlock'
        ? 'Изображение с маркером'
        : block.type === 'TextBlock'
          ? block.content.slice(0, 50) || 'Текст'
          : block.type === 'InputField' || block.type === 'Button' || block.type === 'DragZone'
            ? block.label
            : block.label || 'Объект';
  ...
  return (
    <div
      ...
      onClick={onSelect}
    >
      <span {...attributes} {...listeners} className="cursor-grab text-slate-400 active:cursor-grabbing" onClick={(event) => event.stopPropagation()}>
        ⠿
      </span>
      <span className="text-xl">{icons[block.type]}</span>
      <span className="flex-1 truncate text-sm text-slate-700">{summary}</span>
      <button type="button" onClick={(event) => { event.stopPropagation(); onDelete(); }} className="text-sm text-red-400 hover:text-red-600">
        ✕
      </button>
    </div>
  );
