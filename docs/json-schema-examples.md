...
Изображение, на котором пользователь отмечает определённое место меткой.
...
```json
{
  ...
  "order": 5,
  "src": "https://example.com/plan.png",
  "alt": "План помещения",
  "maxImageWidth": 600,
  "maxImageHeight": 800,
  "viewer": true,
...
**viewer** — должен быть включён: перемещение метки доступно только в полноэкранном режиме.  
**correctX / correctY** — эталонная позиция в процентах (0–100).  
**errorPercent** — допустимое отклонение (евклидово расстояние от эталона, в процентах).  
**markerColor / markerSizePercent** — цвет и размер метки (% меньшей стороны изображения).  
**markerImmediateDrag** — `true`: метку можно тянуть сразу, без клика-активации;
`false`/не задано — требуется первый тап.

Поведение для пользователя:

- во встроенном виде метка показывает текущую позицию (изначально — центр фото);
- в полноэкранном просмотре при выключенном `markerImmediateDrag` первый тап
  по метке активирует её на 3 секунды — за это время нужно нажать и перетащить
  метку; при включённом — метка перетаскивается первым же касанием;
  отпускание пальца фиксирует позицию;
  при зуме и панораме метка остаётся привязана к точке на изображении;
- отправка блокируется с подсказкой, пока метку ни разу не переместили;
- на карточку поддерживается один маркерный блок (первый в схеме).
