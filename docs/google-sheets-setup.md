...
`repost_check_unconfigured`. Для настройки:
...
5. Добавьте свойства:
   ...
   - `ADMIN_PASSWORD_HASH` = тот же SHA-256 хеш пароля админки, что в
     ...
     «Перепроверить репосты» на странице статистики.
6. Сохраните и переиздайте веб-приложение (Deploy → Manage deployments → New version).

Кнопка «Перепроверить репосты» (/admin/stats) пересчитывает has_reposted во
всех ответах по факту: пользователь мог репостнуть позже отправки. Обновляются
только изменившиеся строки; повторный вызов идемпотентен.
