...
- [Что где хранится (localStorage / таблица / память)](docs/data-storage.md)
...
- **Сервер-как-источник-истины**: статусы «Выполнено», подписка на уведомления,
  репосты и время первого просмотра карточки хранятся в Google Sheets.
- **Новый лист Opens** и действие `markCardOpen`: время первого просмотра
  карточки фиксируется на сервере (один раз для пары vk_id + card_id);
  дельта решения считается от него. Локальное значение — только офлайн-fallback.
- **Тихая синхронизация при запуске**: возвращающиеся пользователи автоматически
  проходят checkUser + getAnsweredCards — строки в Users больше не теряются,
  статусы карточек всегда с сервера (см. docs/data-storage.md).
- **Хранилище v2**: локальные ключи получили версионированный префикс
  `vk_contest_v2_`, ключи прошлых версий удаляются при первом запуске;
  из localStorage удалены кеш репостов, флаги «ответил» и уведомлений.
- **Кеширование карточек**: клиентский кеш сокращён до 2 минут; CacheService
  на стороне Apps Script (TTL 5 мин) сохранён.
- **Логи**: буфер ограничен 200 событиями с автоматическим сбросом на сервер
  при переполнении; marker_move пишется не чаще раза в секунду (последняя
  позиция досылается); в лист Logs пишется одна строка на отправку —
  вся пачка событий одной JSON-строкой в ячейке «log».
...
- **Блок-маркер (ImageMarkerBlock)**: рантайм в квизе — метка на фото (изначально центр),
  перемещение в полноэкранном просмотре с защитой от случайных тапов (тап → 3 с на начало
  перетаскивания) либо мгновенное перетаскивание — переключатель `markerImmediateDrag`
  в конструкторе; отпускание фиксирует, привязка к точке фото при зуме/панораме; координаты
  проверяются сервером (см. «Серверная проверка ответов»).
