...
- refreshReposts: строки Answers, чей card_id отсутствует в Cards (или у карточки нет post_id), больше НЕ перезаписываются — сохраняют прежнее значение has_reposted (решение владельца).
- Запись выполняется батчами setValues по непрерывным диапазонам между сиротами (эффективнее построчного setValue), LockService и abort-before-write при ошибке VK сохранены.
- В сводку добавлено поле skippedUnknown (прокинуто в RepostRefreshSummary и тост кнопки: «пропущено без карточки: K» при K>0).

### Изменённые файлы

- `apps-script/Code.gs`, `src/types/index.ts`, `src/components/admin/RefreshRepostsButton.tsx`, `.agent/CHANGELOG.md`
